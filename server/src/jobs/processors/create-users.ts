...
export const createUsersProcessor: Processor = async ({ client, snapshotPayload }) => {
  ...
  for (const user of team.users) {
    ...
    if (ROLES_REQUIRING_EXTENSION.has(user.role) && !user.extension.trim()) {
      throw new Error(
        `Usuário "${user.name}" (${user.role}) precisa de um ramal para ser criado`,
      );
    }

    const found = existing.find((candidate) => candidate.username === user.username);
    if (found) {
      // Sincroniza nome/perfil/ramal num usuário já existente em vez de só
      ...
      await users.updateUser(client, found.id, {
        ...
        ...(user.extension.trim() ? { sipuser: user.extension.trim() } : {}),
      });
